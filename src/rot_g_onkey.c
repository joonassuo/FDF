...
/*   Updated: 2020/03/10 12:51:20 by jsuonper         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				rot_g_onkey(int keycode, t_mlx_struct *coords_ptr)
{
	...
	if (keycode == 126 || keycode == 125)
	{
		...
		rot_grid(coords_ptr->coords, coords_ptr->size, angle, 'x');
	}
	else if (keycode == 124 || keycode == 123)
	{
		angle = (keycode == 124) ? 0.05 : -0.05;
		rot_grid(coords_ptr->coords, coords_ptr->size, angle, 'y');
	}
	else if (keycode == 43 || keycode == 47)
	{
		angle = (keycode == 43) ? 0.05 : -0.05;
		rot_grid(coords_ptr->coords, coords_ptr->size, angle, 'z');
	}
	else if (keycode == 53)
		exit(0);
	mlx_clear_window(coords_ptr->mlx_ptr, coords_ptr->win_ptr);
	draw_grid(coords_ptr->mlx_ptr, coords_ptr->coords, coords_ptr->size);
	return (keycode);
}
